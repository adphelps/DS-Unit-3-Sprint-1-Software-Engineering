...
def train_val_test_split(df):
    temp, test = train_test_split(
        df, test_size=0.2, train_size=0.8, random_state=200)
    train, val = train_test_split(
        temp, test_size=0.2, train_size=0.8, random_state=200)
...
class Dog(Animal):
    ...
    def get_time_last_fed(self):
        time_sec_since_last_time_fed = (
            datetime.datetime.now() - self.last_time_fed).total_seconds()
